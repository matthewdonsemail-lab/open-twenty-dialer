/**
 * Migration script: Add coldCallStatus field to agencyLeads in Twenty
 * 
 * This adds a custom SELECT field for cold calling workflow statuses,
 * separate from the built-in status field used by the SMS pipeline.
 * 
 * Usage: npx tsx backend/scripts/add-cold-call-status-to-leads.ts
 */
...
async function ensureColdCallStatusField() {
  ...
  console.log("Looking for agencyLead object...");
  ...
  const leadObject = objects.find((obj: any) => obj.nameSingular === "agencyLead");
  
  if (!leadObject) {
    throw new Error("agencyLeads object not found in Twenty");
  }
  
  console.log(`Found agencyLead object: ${leadObject.id}`);
  
  // Check if field already exists
  const existingFieldNames = (leadObject.fieldsList || []).map((field: any) => field.name);
