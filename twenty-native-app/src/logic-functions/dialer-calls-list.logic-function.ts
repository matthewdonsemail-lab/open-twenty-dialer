...
import { DIALER_CALLS_LIST_FUNCTION_UNIVERSAL_IDENTIFIER } from 'src/constants/universal-identifiers';
...
const handler = async (event: RoutePayload) => {
  ...
  const payload = (await getRest().get('/rest/agencyCalls', { query: q })) as { data?: Record<string, unknown[]>; totalCount?: number };
  const records = listRows(payload, 'agencyCalls');
...
export default defineLogicFunction({
  universalIdentifier: DIALER_CALLS_LIST_FUNCTION_UNIVERSAL_IDENTIFIER,
  name: 'dialer-calls-list',
  description: 'List Calls (limit/filter/startingAfter keyset)',
  timeoutSeconds: 15,
  handler,
