...
import { DIALER_PHONES_LIST_FUNCTION_UNIVERSAL_IDENTIFIER } from 'src/constants/universal-identifiers';
...
const handler = async (event: RoutePayload) => {
  ...
  const payload = (await getRest().get('/rest/agencyPhones', { query: q })) as { data?: Record<string, unknown[]>; totalCount?: number };
  const records = listRows(payload, 'agencyPhones');
...
export default defineLogicFunction({
  universalIdentifier: DIALER_PHONES_LIST_FUNCTION_UNIVERSAL_IDENTIFIER,
  name: 'dialer-phones-list',
  description: 'List Phones (limit/filter/startingAfter keyset)',
  timeoutSeconds: 15,
  handler,
