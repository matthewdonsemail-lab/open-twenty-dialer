...
import { DIALER_LEADS_LIST_FUNCTION_UNIVERSAL_IDENTIFIER } from 'src/constants/universal-identifiers';
...
const handler = async (event: RoutePayload) => {
  ...
  const payload = (await getRest().get('/rest/agencyLeads', { query: q })) as { data?: Record<string, unknown[]>; totalCount?: number };
  const records = listRows(payload, 'agencyLeads');
...
export default defineLogicFunction({
  universalIdentifier: DIALER_LEADS_LIST_FUNCTION_UNIVERSAL_IDENTIFIER,
  name: 'dialer-leads-list',
  description: 'List Leads (limit/filter/startingAfter keyset)',
  timeoutSeconds: 15,
  handler,
