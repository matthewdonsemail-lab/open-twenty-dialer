...
import { DIALER_CAMPAIGNS_LIST_FUNCTION_UNIVERSAL_IDENTIFIER } from 'src/constants/universal-identifiers';
...
const handler = async (event: RoutePayload) => {
  ...
  const payload = (await getRest().get('/rest/agencyCampaigns', { query: q })) as { data?: Record<string, unknown[]>; totalCount?: number };
  const records = listRows(payload, 'agencyCampaigns');
...
export default defineLogicFunction({
  universalIdentifier: DIALER_CAMPAIGNS_LIST_FUNCTION_UNIVERSAL_IDENTIFIER,
  name: 'dialer-campaigns-list',
  description: 'List Campaigns (limit/filter/startingAfter keyset)',
  timeoutSeconds: 15,
  handler,
