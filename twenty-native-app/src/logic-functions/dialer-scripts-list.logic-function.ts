...
import { DIALER_SCRIPTS_LIST_FUNCTION_UNIVERSAL_IDENTIFIER } from 'src/constants/universal-identifiers';
...
const handler = async (event: RoutePayload) => {
  ...
  const payload = (await getRest().get('/rest/agencyScripts', { query: q })) as { data?: Record<string, unknown[]>; totalCount?: number };
  const records = listRows(payload, 'agencyScripts');
...
export default defineLogicFunction({
  universalIdentifier: DIALER_SCRIPTS_LIST_FUNCTION_UNIVERSAL_IDENTIFIER,
  name: 'dialer-scripts-list',
  description: 'List Scripts (limit/filter/startingAfter keyset)',
  timeoutSeconds: 15,
  handler,
